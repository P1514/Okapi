...
public class HeatMap extends ResultTable {
	...
	/*
	 * name and ID of the context element for which a heatmap is requested. ex: for
	 * machine id = 2, this field would have the name of the machine number 2
	 */
	...
	private static Logger LOGGER = new Logging().create(HeatMap.class.getName(), false);

	public HeatMap(Integer kpiId, TableValueType type, SamplingInterval granularity, Timestamp startTime,
			Timestamp endTime, Integer contextElementId, TableValueType varXAxis, TableValueType varYAxis) {
	...
	public String getHeatMapQueryString(String aggregation) {
		...
		String query = "SELECT vx.\"NAME\" as \"varX\", vy.\"NAME\" as \"varY\", " + valueColumnStr + " as \"value\""
				+ " FROM \"KPI_VALUES\" kv" + "	INNER JOIN \"" + varXStr.toUpperCase() + "\" vx ON \""
				+ varXStr.toUpperCase() + "_ID\" = vx.\"ID\"" + " INNER JOIN \"" + varYStr.toUpperCase() + "\" vy ON \""
				+ varYStr.toUpperCase() + "_ID\" = vy.\"ID\"" + " WHERE kv.\"KPI_TIMESTMP\" BETWEEN TIMESTAMP('"
				+ startTime + "') AND TIMESTAMP('" + endTime + "')"
				+ getContextElementWhereClause(super.tableVT, contextStr, contextElementId, false).toUpperCase()
				+ " AND \"KPI_ID\" = " + kpiId + " AND \"" + varXStr.toUpperCase() + "_ID\" IS NOT NULL" + " AND \""
				+ varYStr.toUpperCase() + "_ID\" IS NOT NULL" + groupByClauseStr + " ORDER BY \"varX\", \"varY\";";

		LOGGER.log(Level.INFO,"HeatMap Query : " + query);
		return query;
	}
	...
	private String getSamplingIntervalWhereClause(SamplingInterval granularity, Timestamp time, boolean mainClause) {
		...
		case DAILY:
			...
			// result += "DAY(CAST(kv.\"kpi_timestmp\" AS DATE)) =
			// DAY(CAST(TIMESTAMP'"+time+"' AS DATE)) ";
			break;
		case HOURLY:
			result += "HOUR(CAST(kv.\"KPI_TIMESTMP\" AS TIME)) = HOUR(CAST(TIMESTAMP'" + time + "' AS TIME)) ";
			// result += "HOUR(CAST(kv.\"kpi_timestmp\" AS TIME)) =
			// HOUR(CAST(TIMESTAMP'"+time+"' AS TIME)) ";
			break;
		case MONTHLY:
			result += "MONTH(CAST(kv.\"KPI_TIMESTMP\" AS DATE)) = MONTH(CAST(TIMESTAMP'" + time + "' AS DATE)) ";
			// result += "MONTH(CAST(kv.\"kpi_timestmp\" AS DATE)) =
			// MONTH(CAST(TIMESTAMP'"+time+"' AS DATE)) ";
			break;
		case WEEKLY:
			result += "MONTH(CAST(kv.\"KPI_TIMESTMP\" AS DATE)) = MONTH(CAST(TIMESTAMP'" + time + "' AS DATE)) ";
			// result += "MONTH(CAST(kv.\"kpi_timestmp\" AS DATE)) =
	...
	private String getContextElementWhereClause(TableValueType contextualInformation, String contextStr,
			Integer contextElmentId, boolean mainClause) {
		String result = "";
	...
	public void setHeatMapValues(HeatMap heatMapA, HeatMap heatMapB, String operator, Integer xSize, Integer ySize) {
		...
		LOGGER.log(Level.INFO, "xSize:" + xSize + ";ySize:" + ySize);
		...
		LOGGER.log(Level.INFO,"heatMapA.resultsRows.size():" + heatMapA.resultsRows.size());

		LOGGER.log(Level.INFO,"heatMapB.resultsRows.size():" + heatMapB.resultsRows.size());
		...
		for (int i = 0; i < heatMapA.resultsRows.size(); i++) {
			...
			System.out.println("i=(" + i + ")" + "kpiId(" + kpiId + ")" + "for <"
					+ heatMapA.resultsRows.get(i).columnValues[0] + ">" + "index: " + varXPosA);

			varYPosA = heatMapA.varYUnique.indexOf(heatMapA.resultsRows.get(i).columnValues[1]);
			System.out.println("i=(" + i + ")" + "kpiId(" + kpiId + ")" + "for <"
					+ heatMapA.resultsRows.get(i).columnValues[1] + ">" + "index: " + varYPosA);

			varXPosB = heatMapB.varXUnique.indexOf(heatMapB.resultsRows.get(i).columnValues[0]);
			System.out.println("i=(" + i + ")" + "kpiId(" + kpiId + ")" + "for <"
					+ heatMapB.resultsRows.get(i).columnValues[0] + ">" + "index: " + varXPosB);

			varYPosB = heatMapB.varYUnique.indexOf(heatMapB.resultsRows.get(i).columnValues[1]);
			System.out.println("i=(" + i + ")" + "kpiId(" + kpiId + ")" + "for <"
					+ heatMapB.resultsRows.get(i).columnValues[1] + ">" + "index: " + varYPosB);
			...
			_heatMap[varXPosA][varYPosA] = Double
					.toString(math.getResult(_heatMapA[varXPosA][varYPosA], _heatMapB[varXPosA][varYPosA], operator));
			LOGGER.log(Level.INFO,"i=(" + i + ")" + "valueA:" + _heatMapA[varXPosA][varYPosA] + ";" + "valueB:"
					+ _heatMapB[varXPosA][varYPosA] + ";  " + operator);

			LOGGER.log(Level.INFO,"i=(" + i + ")" + "valueA:" + _heatMapA[varXPosA][varYPosA] + ";" + "valueB:"
					+ _heatMapB[varXPosA][varYPosA] + ";" + "result: " + value);
		}
	...
	public Object toJSonObjectHeatMap() {
		...
		for (int i = 0; i < varXUnique.size(); i++) {
			for (int j = 0; j < varYUnique.size(); j++) {
				tempArrStr
						.add("{\"varX\":" + (i + 1) + ", \"varY\":" + (j + 1) + ", \"value\":" + _heatMap[i][j] + "}");
			}
		}


		try {
			jsonObject = new JSONObject(tempArrStr.toString());
		} catch (JSONException e) {
			LOGGER.log(Level.WARNING, Errors.getError(8));
		}
	...
	private Object toJsonObjectHeatMapLabels(ArrayList<String> labels) {
		...
		try {
			jsonObject = new JSONObject(Arrays.toString(x));
		} catch (JSONException e) {
			LOGGER.log(Level.WARNING, Errors.getError(8));
		}
		return jsonObject;
